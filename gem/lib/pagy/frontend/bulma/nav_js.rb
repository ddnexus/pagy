...
class Pagy
  Frontend.module_eval do
    ...
    def pagy_bulma_nav_js(pagy, classes: 'pagination is-centered', **)
      ...
      tokens = { before:  %(#{bulma_previous_next_html(pagy, create_anchor)}<ul class="pagination-list">),
                 anchor:  %(<li>#{create_anchor.(PAGE_TOKEN, LABEL_TOKEN, classes: 'pagination-link')}</li>),
