...
require_relative 'pagy/shared'

class Pagy
  # Frontend modules are optimized for performance.
  ...
  Frontend.class_eval do
    ...
    def pagy_nav(pagy, id: nil, aria_label: nil, **vars)
      ...
      html = %(<nav#{id} class="pagy nav" #{pagy_nav_aria_label(pagy, aria_label:)}#{data}>#{prev_a(pagy, a)})
