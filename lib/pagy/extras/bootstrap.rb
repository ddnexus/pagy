...
class Pagy # :nodoc:
  ...
  module BootstrapExtra
    ...
    def pagy_bootstrap_nav(pagy, pagy_id: nil, link_extra: '', nav_class: '', **vars)
      p_id = %( id="#{pagy_id}") if pagy_id
      link = pagy_link_proc(pagy, link_extra: %(class="page-link" #{link_extra}))

      html = +%(<nav#{p_id} class="pagy-bootstrap-nav #{nav_class}" aria-label="pager"><ul class="pagination">)
