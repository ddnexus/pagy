    ...
    def pagy_tailwind_nav(pagy)
      ...
      pagy.series.each do |item| # series example: [1, :gap, 7, 8, "9", 10, 11, :gap, 36]
        html << if    item.is_a?(Integer); %(<li class="block hover:text-white hover:bg-blue-600 text-blue border-r border-blue-400 px-3 py-2">#{link.call item}</li>)                                                               # page link
                ...
                elsif item == :gap       ; %(<li class="block hover:text-white hover:bg-blue-600 text-blue border-r border-blue-400 px-3 py-2 cursor-not-allowed"><a href="#">#{pagy_t('pagy.nav.gap')'}</a></li>) # page gap
                end
      end
