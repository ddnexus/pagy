  ...
  module BulmaExtra
    ...
    def pagy_bulma_nav(pagy, pagy_id: nil, link_extra: '')
      ...
      pagy.series.each do |item| # series example: [1, :gap, 7, 8, "9", 10, 11, :gap, 36]
        html << case item
                when Integer then %(<li>#{link.call item, item, %(class="pagination-link" aria-label="goto page #{item}")}</li>)
                when String  then %(<li>#{link.call item, item, %(class="pagination-link is-current" aria-label="page #{item}" aria-current="page")}</li>)
                when :gap    then %(<li><span class="pagination-ellipsis">#{pagy_t 'pagy.nav.gap'}</span></li>)
                end
      end
      html << %(</ul></nav>)
    end
