...
- The `:offset` variable used by the `Pagy::Calendar::Week`: set the `:first_weekday` variable to the symbol of the first day of the week (e.g.: `first_weekday: :sunday`) instead.
- The `:first_weekday` default is `:sunday` in `5.*` to avoid breaking apps, but it will be `:monday` from `6.0` on, in order to be consistent with the ISO-8601 standard (and Rails).
