...
The `limit_selector_js_tag` allows the user to select any arbitrary limit per page, up to the `:max_limit` option. It raises an `OptionError` exception if the `:max_limit` is not truthy.
...
```ruby
>> puts @pagy.limit_selector_js_tag(max_limit: 100)
...
>> puts @pagy.limit_selector_js_tag(max_limit: 100, id: 'my-elector', item_name: 'Products')
