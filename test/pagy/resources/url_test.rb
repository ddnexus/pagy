...
describe 'pagy/helpers/url' do
  ...
  describe '#pagy_page_url' do
    it 'renders basic url' do
      pagy, = app.send(:pagy_offset, @collection, count: 1000, page: 3)
      _(pagy.send(:page_url, 5)).must_equal '/foo?page=5'
      _(pagy.send(:page_url, 5, absolute: true)).must_equal 'http://example.com:3000/foo?page=5'
    end
    it 'renders url with params' do
      pagy, = app.send(:pagy_offset, @collection, count: 1000, page: 3, params: { a: 3, b: 4 })
      _(pagy.send(:page_url, 5)).must_equal "/foo?page=5&a=3&b=4"
      _(pagy.send(:page_url, 5, absolute: true)).must_equal "http://example.com:3000/foo?page=5&a=3&b=4"
    end
    it 'renders url with fragment' do
      pagy, = app.send(:pagy_offset, @collection, count: 1000, page: 3)
      _(pagy.send(:page_url, 6, fragment: '#fragment')).must_equal '/foo?page=6#fragment'
      _(pagy.send(:page_url, 6, absolute: true, fragment: '#fragment')).must_equal 'http://example.com:3000/foo?page=6#fragment'
    end
    it 'renders url with params and fragment' do
      pagy, = app.send(:pagy_offset, @collection, count: 1000, page: 3, params: { a: 3, b: 4 })
      _(pagy.send(:page_url, 5, fragment: '#fragment')).must_equal "/foo?page=5&a=3&b=4#fragment"
      _(pagy.send(:page_url, 5, absolute: true, fragment: '#fragment')).must_equal "http://example.com:3000/foo?page=5&a=3&b=4#fragment"
    end
    it 'renders url with overridden path' do
      pagy, = app.send(:pagy_offset, @collection, count: 1000, page: 3, request_path: '/bar')
      _(pagy.send(:page_url, 5)).must_equal '/bar?page=5'
    end
    it 'raises OptionError for missing @request' do
      pagy, = Pagy::Offset.new(count: 1000, page: 3)
      _ { pagy.send(:page_url, 1) }.must_raise Pagy::OptionError
    end
  end
  ...
  describe 'process pagy_params' do
    it 'overrides params' do
      ...
      _(pagy.send(:page_url, 5, fragment: '#fragment')).must_equal "/foo?a=5&page=5&b=4&add_me=add_me#fragment"
    end
  end
