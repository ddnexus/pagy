...
describe 'pagy/helpers/url' do
  ...
  describe '#pagy_page_url' do
    ...
    it 'renders url with options[:request]' do
  ...
  describe 'pagy query_utils' do
    ...
    it "handles nil and '' values" do
      _("res: #{Pagy::Url::QueryUtils.build_nested_query({ b: nil })}").must_equal "res: b"
      _("res: #{Pagy::Url::QueryUtils.build_nested_query({ a: { b: { c: nil } } })}").must_equal "res: a%5Bb%5D%5Bc%5D"
      _(Pagy::Url::QueryUtils.build_nested_query({ b: '' })).must_equal "b="
      _(Pagy::Url::QueryUtils.build_nested_query({ a: { b: { c: '' } } })).must_equal "a%5Bb%5D%5Bc%5D=" # "a[b][c]=''"
    end
